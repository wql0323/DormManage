...
public class RecordServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if("preSave".equals(action)) {
			...
			recordDetails(request, response);
		...
		try {
			...
			if("admin".equals((String)currentUserType)) {
				...
				List<Record> recordList = RecordDao.recordListWithNumber(con, record, student.getStuNum());
				request.setAttribute("recordList", recordList);
	...
	private void recordSave(HttpServletRequest request,
		...
		Record record = new Record(Integer.parseInt(recordId),stuNum, date, detail);
		...
		try {
			...
			DormManager manager = (DormManager)(session.getAttribute("currentUser"));
			int buildId = manager.getDormBuildId();
			Student student = StudentDao.getNameById(con, stuNum, buildId);
			if(student.getName() == null) {
				...
				if(StringUtil.isNotEmpty(recordId) && Integer.parseInt(recordId)!=0) {
					saveNum = recordDao.recordUpdateORAdd(con, record);
				}
				if(saveNum > 0) {
					request.getRequestDispatcher("record?action=list").forward(request, response);
				} else {
	...
	private void recordDetails(HttpServletRequest request,
							   HttpServletResponse response){
		Record record = new Record();
		String stuNum = request.getParameter("stuNum");
		Connection con=null;
		try{
			con=dbUtil.getCon();
			List<Record>recordListWithNumber= RecordDao.recordListWithNumber(con,record,stuNum);
			request.setAttribute("stuNum",stuNum);
			request.setAttribute("recordListWithNumber",recordListWithNumber);
			request.setAttribute("mainPage","dormManager/recordDetails.jsp");
			request.getRequestDispatcher("mainManager.jsp").forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
		}
