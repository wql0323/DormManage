...
//		}
	...
	public int dormManagerCount(Connection con, DormManager s_dormManager)throws Exception {
		StringBuffer sb = new StringBuffer("select count(*) as total from dbo.t_dormManager t1");
		if(StringUtil.isNotEmpty(s_dormManager.getName())) {
			sb.append(" where t1.name like '%"+s_dormManager.getName()+"%'");
		} else if(StringUtil.isNotEmpty(s_dormManager.getUserName())) {
			sb.append(" where t1.userName like '%"+s_dormManager.getUserName()+"%'");
	...
	public int dormManagerAdd(Connection con, DormManager dormManager)throws Exception {
		...
		PreparedStatement pstmt=con.prepareStatement(sql);
		pstmt.setInt(1, dormManager.getDormManagerId());
	...
	public int dormManagerupdateORadd(Connection con, DormManager dormManager)throws Exception
	{
		int r=0;   //确定返回值
		String sql="select * from t_dormManager t1 where dormManId = ?";
		PreparedStatement pstmt=con.prepareStatement(sql);
		pstmt.setInt(1, dormManager.getDormManagerId());
		ResultSet rs= pstmt.executeQuery();

		/*
		通过判断dormManId是否存在，选择调用函数的类型
		如果dormManId已经存在，rs.next()为true，调用更新函数
		如果dormManId不存在，rs.next()为false，调用添加函数
		 */

		if(rs.next()){
			r=dormManagerUpdate(con,dormManager);
		}
		else{
			r=dormManagerAdd(con,dormManager);
		}
		return r;
	}
