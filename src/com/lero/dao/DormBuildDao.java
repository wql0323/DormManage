...
//		}
	...
	public int dormBuildAdd(Connection con, DormBuild dormBuild)throws Exception {
		String sql = "insert into t_dormBuild values(?,?,?)";
		PreparedStatement pstmt=con.prepareStatement(sql);
		pstmt.setInt(1, dormBuild.getDormBuildId());
		pstmt.setString(2, dormBuild.getDormBuildName());
		pstmt.setString(3, dormBuild.getDetail());
...
//		String sql="select * from t_dormBuild,t_dormManager,t_connection where dormManId=managerId and dormBuildId=buildId and dormBuildId=?";
	...
	public int dormBuildUpdateORAdd(Connection con,DormBuild dormBuild) throws Exception{
		int r=0;
		String sql="select * from t_dormbuild t1 where t1.dormBuildId = ?";
		PreparedStatement pstmt=con.prepareStatement(sql);
		pstmt.setInt(1,dormBuild.getDormBuildId());
		ResultSet rs = pstmt.executeQuery();
		/*
		通过判断dormBuildId是否存在，选择调用函数的类型
		如果dormBuildId已经存在，rs.next()为true，调用更新函数
		如果dormBuildId不存在，rs.next()为false,调用添加函数
		 */
		if(rs.next())
		{
			r=dormBuildUpdate(con,dormBuild);
		}
		else{
			r=dormBuildAdd(con,dormBuild);
		}
		return r;
	}
}
